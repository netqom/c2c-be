"use strict";
// Class definition
var KTAppsDatatableFeature = function() {
	var _demo = function(pSize) { 
		var datatable = $('#kt_feature_datatable').KTDatatable({
			...
			columns: [
				...
				},
				{
					...
					template: function(data) {
						return '<span class="font-weight-bolder"><img src="'+BASE_URL+'/storage/' + data.image +'" height="50px;" width="50px;" alt="product-image"></span>';
					}
				},
                 {
                    field: 'description',
				...
				// },
				{
					...
					textAlign: 'left',
					template: function(data) {
						var ret = '<a class="btn btn-sm btn-light-warning btn-icon" href="javascript:;" onclick="addFeature('+data.id+')"  title="Edit Feature"><i class="icon-1x text-dark-50 flaticon-edit"></i></a>&nbsp&nbsp\n\
                        <a class="btn btn-sm btn-light-danger btn-icon" href="javascript:;" onclick="deleteFeature('+data.id+');" title="Delete Feature"><i class="icon-1x text-dark-50 flaticon-delete"></i></a>';
	...
	return {
		...
		reload: function() {
		   // datatable.reload();
			$('#kt_feature_datatable').KTDatatable('reload');
		}
	};
}();

jQuery(document).ready(function() {
	KTAppsDatatableFeature.init();
});
...
function validateFeaturesForm(form_id)
{
	var form = $("#"+form_id);
    form.validate();
    if (form.valid()) {
       infoLoadingBox();
        ...
        $.ajax({
            method: "POST",
            url: BASE_URL + "/admin/features/add-update",
            data: formData,
